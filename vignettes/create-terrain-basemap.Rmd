...
adm0 <- gisr::get_admin0(countries = countryname) %>% 
  select(sovereignt, admin, iso_a2, iso_a3)

adm1 <- gisr::get_admin1(countries = countryname) %>% 
  select(name, code_hasc)
...
adm1 %>% 
  gisr::gview() + 
  geom_sf_text(data = adm1, aes(label = name), size = 3)
```